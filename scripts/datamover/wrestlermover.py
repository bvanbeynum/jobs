...
if len(portalEvents) > 0:
	...
	logMessage(f"	No events to update")
...
while True:
	...
	for wrestlerRow in wrestlersBatch:
		wrestler = {
			...
			"searchNames": json.loads(wrestlerRow.SearchNames) if wrestlerRow.SearchNames else json.loads([]),
			"searchTeams": json.loads(wrestlerRow.SearchTeams) if wrestlerRow.SearchTeams else json.loads([]),
			"states": json.loads(wrestlerRow.States) if wrestlerRow.States else json.loads([]),
